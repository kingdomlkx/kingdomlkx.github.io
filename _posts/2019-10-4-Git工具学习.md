...
44. __git config --global alias.[st] [status]__  
配置别名.  

45. __git config --global alias.unstage 'reset HEAD'__  
...
46.  __git config --global alias.last 'log -1'__  
配置一个git last，让其显示最后一次提交信息.  

47. __git config --global alias.lg "log --color --graph --pretty=format:'%Cred%h%Creset -%C(yellow)%d%Creset %s %Cgreen(%cr) %C(bold blue)<%an>%Creset' --abbrev-commit"__
某位丧心病狂的强者配置的别名  
git lg
